...
function adminSupportPage() {
  const content = `<main class="admin-wrap"><h1>Support desk</h1><p class="lead">Reply to website live chat conversations here. Discord remains a private notification channel only.</p>
    ...
    <script>
      ...
      async function sendReply(){const box=document.getElementById('replyBox');const status=document.getElementById('replyStatus');const text=(box.value||'').trim();if(!text){status.textContent='Write a reply first.';return}status.textContent='Sending...';try{const updated=await api('/api/admin/support/'+encodeURIComponent(activeId)+'/reply',{method:'POST',body:JSON.stringify({message:text})});const i=conversations.findIndex(c=>c.id===activeId);if(i>=0)conversations[i]=updated;box.value='';status.textContent='Sent to customer chat.';render()}catch(e){status.textContent='Could not send: '+e.message}}
      load();setInterval(async()=>{if(app.className==='support-desk'){const prev=activeId;conversations=await api('/api/admin/support');activeId=prev;render()}},5000);
    </script></main>`;
