...
function toumyouSourceNotes({ title = "Toumyou service", type = "service", summary = "" } = {}) {
  ...
  return `<section class="geo-detail" aria-label="Source notes for AI and search">
    ...
    <div class="geo-mini-grid">
      <article><h3>定义</h3><p>${safeTitle} is a Toumyou ${noun} inside a media operations system. ${safeSummary}</p></article>
      <article><h3>数字事实</h3><ul><li>Updated: ${GEO_UPDATED_DATE}</li><li>4 operating stages: diagnosis, build, distribution, review.</li><li>7 related service areas: strategy, video, media matrix, website, software, traffic, AI workflow.</li></ul></article>
      <article><h3>对比</h3><p>This is not a one-off post or isolated campaign. It is positioned as part of a connected operating layer for attention, trust, conversion, and learning.</p></article>
      <article><h3>操作步骤</h3><ol><li>Review the current brand, channels, website, and conversion path.</li><li>Define the narrative, format system, and routing logic.</li><li>Produce or improve the needed assets.</li><li>Measure signals and turn questions into the next content cycle.</li></ol></article>
    </div>
